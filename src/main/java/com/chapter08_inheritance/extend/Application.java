...
public class Application {
    public static void main(String[] args) {
        ...
        모든 객체는 자신이 수신한 메시지에 대해 응당을 해야하는 책임을 가지며, 그 책임의 규모는 적절해야한다.
        ...
        부모 클래스를 추상화하는 경우에는 역할의 관점을 바라봐야 한다.
        그래야 자식 클래스로 생성한 객체들이 서로 역할을 수정해가며 유연한 코드를 작성할 수 있게 된다.
        동일한 역할을 가지는 모든 객체는 동일한 메시지를 수신하기는 하지만,
        객체별로 그 메시지에 응답하는 방식은 서로 다를 수 있다.(다형성)

        자식은 부모 클래스에 접근하여 사용할 수 있지만
        부모는 자식 클래스에 접근할 수 없다.
        */

        System.out.println("차의 동작======");
        Car car = new Car();
        car.soundHorn();
        car.run();
        car.soundHorn();
        ...
        System.out.println("소방차의 동작======");
        FireCar fireCar = new FireCar();
        fireCar.soundHorn();
        fireCar.run(); // 상속을 받고 있기 때문에 부모 클래스의 메소드를 사용할 수 있다.
        fireCar.soundHorn();
        fireCar.stop();
        fireCar.soundHorn();
//        fireCar.runningStatus; // 상속을 받고 있지만 부모 클래스의 private 필드에는 접근할 수 없다.
        fireCar.sprayWater(); // 상속은 확장이다. 추가적인 기능도 작성할 수 있다.

        // 안 좋은 예시 : Car의 모든 메소드를 오버라이딩하기 때문에 불필요한 것까지 사용하게 된다. 차라리 새로 하나 만드는게 낫다.
        System.out.println("레이싱카의 동작======");
